...
DriveTrain::DriveTrain()
    : m_leftMotorA{kDrivetrainMotorLeftAPort, rev::CANSparkMax::MotorType::kBrushless},
      m_leftMotorB{kDrivetrainMotorLeftBPort, rev::CANSparkMax::MotorType::kBrushless},
      m_leftEncoder{m_leftMotorA.GetEncoder()},
      m_rightMotorA{kDrivetrainMotorRightAPort, rev::CANSparkMax::MotorType::kBrushless},
  ...
  frc::sim::SimDeviceSim leftDeviceSim(("SPARK MAX [" + std::to_string(m_leftMotorA.GetDeviceId()) + "]").c_str());
  m_leftEncoderPositionSim = leftDeviceSim.GetDouble("Position");
  m_leftEncoderVelocitySim = leftDeviceSim.GetDouble("Velocity");
  
  frc::sim::SimDeviceSim rightDeviceSim(("SPARK MAX [" + std::to_string(m_rightMotorA.GetDeviceId()) + "]").c_str());
  std::cout << "Hello: " << rightDeviceSim << ", " << leftDeviceSim << std::endl;
